...
Astryx-specific guidance for this app; see the root `AGENTS.md` for workspace-wide conventions.
Generated by `astryx init --features agents` (re-run after an `@astryxdesign/core` bump) —
everything between the markers is regenerated verbatim, don't hand-edit it.

No override right now: there is no `@acme/ui` wrapper package. Import components straight from
`@astryxdesign/core` (see `astryx component <Name>` below). The reset/base/theme CSS bootstrap
described in SETUP is already wired into `src/styles.css` — don't duplicate it in a new file.

<!-- ASTRYX:START -->

Astryx v0.3.0 · 155 components CLI: run every command as `pnpm exec astryx <cmd>` (shown below as
`astryx ...`).
...
MORE CLI: search "<query>" find any component / hook / doc / template / block component --list 155
